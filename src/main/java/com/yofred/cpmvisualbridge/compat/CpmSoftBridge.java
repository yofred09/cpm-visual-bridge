...
public final class CpmSoftBridge {
        ...
        try {
            ...
            if (definitionLoader != null && definitionLoadPlayer != null
                ...
                Object cpmPlayer = definitionLoadPlayer.invoke(
                        definitionLoader, player.getGameProfile(), "cpmvisualbridge_probe"
                );
    ...
    private static synchronized void initializeDefinitionProbe() {
        ...
        try {
            ...
            CpmVisualBridge.LOGGER.debug("CPM profile definition probe unavailable: {}", t.toString());
        }
    }
    ...
    private static void acceptClientApi(Object api) {
        ...
        try {
            ...
            CpmVisualBridge.LOGGER.info("CPM player renderer ready for auxiliary effects");
        } catch (Throwable t) {
            playerRenderer = null;
    ...
    private static void probeApi() {
        ...
        for (String name : classNames) {
            try {
                ...
                CpmVisualBridge.LOGGER.debug("CPM probe skipped for {}: {}", name, t.toString());
            }
        }
        apiAvailable = false;
    }
    ...
    private static void registerPluginViaImc() {
        try {
            ...
            try {
                ...
                CpmVisualBridge.LOGGER.debug("IMC to cpm failed: {}", t.toString());
            }
            try {
                sent |= InterModComms.sendTo(CpmCompat.MOD_ID_LEGACY, "api", imcPayload);
            } catch (Throwable t) {
                CpmVisualBridge.LOGGER.debug("IMC to customplayermodels failed: {}", t.toString());
            }
            if (sent) {
                CpmVisualBridge.LOGGER.info("Registered CPM soft-compat plugin via InterModComms");
            } else {
                CpmVisualBridge.LOGGER.debug(
                        "CPM IMC send returned false (mod may use @CPMPlugin-only discovery); limb-skip still active"
                );
            }
        } catch (ClassNotFoundException | NoClassDefFoundError e) {
            CpmVisualBridge.LOGGER.debug("ICPMPlugin not on classpath; skipping IMC plugin registration");
        } catch (Throwable t) {
